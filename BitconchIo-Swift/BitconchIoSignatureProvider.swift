//
//  BitconchSignatureProvider.swift
//  AppStructure
//
...
public final class BitconchIoSignatureProvider: EosioSignatureProviderProtocol {
    private struct Key {
    ...
    public init(privateKeys: [String]) throws {
        for privateKey in privateKeys {
            let (_, version, _) = try privateKey.bitconchIoComponents()
            if version != "K1" {
            ...
            let eosioPublicKey = compressedPublicKey.toBitconchIoLegacyPublicKey
    ...
    public func signTransaction(request: EosioTransactionSignatureRequest, completion: @escaping (EosioTransactionSignatureResponse) -> Void) {
        ...
        do {
            ...
            for eosioPublicKey in request.publicKeys {
                let compressedPublicKey = try Data(bitconchPublicKey: eosioPublicKey)
